...
public class GlobalExceptionHandler {
    ...
    /**
     * Handles constraint violations from:
     * - @Validated on controller path/query params  → violations set is populated
     * - ValidationUtils.builder().validate()        → violations set is empty; message is pre-formatted
     */
    ...
    public ResponseEntity<ErrorVO> handleUnexpectedException(Exception ex) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ErrorVO("Something is wrong!"));
    }
}
