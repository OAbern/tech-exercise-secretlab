...
public class KeyValueService {
     ...
     *
     * Concurrency guarantee: the PESSIMISTIC_WRITE lock on the key_value row
     * serialises all concurrent writes to the same key. After a competing
     * transaction commits and releases the lock, the next waiter re-reads the
     * already-updated current_version, so version numbers are always sequential.
     *
     * key_value is updated with the latest value for fast reads;
     * key_value_history always receives an append for point-in-time queries.
     */
    @Transactional
    public KeyValueHistory put(StoreRequest request) {
        ...
        return keyValueRepository.save(new KeyValueHistory(request.getKey(), newVersion, request.getValueJson(), now));
    }

    /**
     * Returns the latest value by reading directly from key_value —
     * no join with key_value_history needed.
     */
    public Optional<KeyValueHistory> getLatest(String key) {
        return keyVersionRepository.findByStoreKey(key)
                .map(kv -> new KeyValueHistory(kv.getStoreKey(), kv.getCurrentVersion(), kv.getValue(), kv.getUpdatedAt()));
    }
