...
public interface KeyValueRepository extends JpaRepository<KeyValue, Long> {
  ...
  /**
   * Acquires a PESSIMISTIC_WRITE (SELECT FOR UPDATE) lock on the key_value row for the given key.
   * Concurrent callers block here until the lock is released, then read the committed
   * current_version — guaranteeing sequential increments.
   */
