...
class ObjectControllerTest {
    ...
    @Autowired
    private KeyValueRepository keyVersionRepository;

    @Autowired
    private KeyValueHistoryRepository keyValueRepository;
    ...
    void getLatest_existingKey_returnsLatestEntry() throws Exception {
        ...
        mockMvc.perform(get("/object/hero"))
                .andExpect(status().isOk())
    ...
    void getLatest_unknownKey_returnsNull() throws Exception {
        mockMvc.perform(get("/object/nonexistent"))
                .andExpect(status().isOk())
    ...
    void getAtTimestamp_returnsCorrectVersion() throws Exception {
        ...
        mockMvc.perform(get("/object/histkey").param("timestamp", String.valueOf(t1)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.version").value(1))
                .andExpect(jsonPath("$.value").value("value1"));

        mockMvc.perform(get("/object/histkey").param("timestamp", String.valueOf(t2)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.version").value(2))
                .andExpect(jsonPath("$.value").value("value2"));
    }
    ...
    void getAtTimestamp_keyNotYetExisted_returnsNull() throws Exception {
        ...
        mockMvc.perform(get("/object/latekey").param("timestamp", String.valueOf(t1)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").doesNotExist());
    }
    ...
    void getAtTimestamp_negativeTimestamp_returns400() throws Exception {
        mockMvc.perform(get("/object/mykey").param("timestamp", "-1"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error").isString());
    }
